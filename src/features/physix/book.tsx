...
export function BookScreen({preview,demo=false,initialMode='in_clinic',initialQuery='',initialService='',onContinue}:{preview:boolean;demo?:boolean;initialMode?:VisitMode;initialQuery?:string;initialService?:string;onContinue?:(serviceId:string,mode:VisitMode)=>void}) {
 ...
 const candidates=preview?serviceCandidates:[];
 ...
 {!available.length&&<div className="px-empty"><Search size={28}/><h2>{preview?'No matching services':'Booking is not open yet'}</h2><p>{preview?'Try a different search or appointment type.':'Published services and live availability will appear here when the clinic is ready.'}</p>{preview&&<button className="button" onClick={()=>{setQuery('');changeMode('in_clinic');}}>Clear filters</button>}</div>}
 ...
 {availability&&selection&&<Sheet title="Availability" onClose={()=>setAvailability(false)}><div className="px-availability"><CalendarDays size={36}/><h3>{selection.name}</h3><p>{mode==='online'?'Online':'In clinic'}</p><h2>Live times aren’t connected yet.</h2><p>This preview does not create an appointment. Availability, fees and clinic policies still need to be configured.</p><Link className="button primary full" href={'/dev/demo/book?service='+selection.id+'&mode='+mode}>Try the booking demo<ArrowRight size={18}/></Link><button className="button full" onClick={()=>setAvailability(false)}>Back to services</button></div></Sheet>}
 </Shell>;
}
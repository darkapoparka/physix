...
export function CareProgress({initialAccount}: {initialAccount?: LocalAccount} = {}) {
  ...
  return <Shell local><div className="px-care-heading"><h1>Your progress</h1><p>Sessions and check-ins.</p></div><CareNavigation active="progress"/>
    ...
    <SectionTitle title={day?'History · '+day:'Session history'} href="/care/programmes" label="Programmes"/>
    {day && <button className="px-text-link" onClick={()=>setDay('')}>Show all days</button>}
    <div className="row-group">{history.length?history.map(s=><Row key={s.id} href={'/care/sessions/'+s.id} icon={s.state==='completed'?<Check size={20}/>:<Clock3 size={20}/>} detail={(s.completed_at||s.started_at).slice(0,10)+' · '+s.state.replaceAll('_',' ')+' · '+Math.floor(s.elapsed_seconds/60)+' min'}>{s.prescription.title}</Row>):<div className="px-empty"><h2>No recorded sessions in this period.</h2><Link className="button" href="/care/programmes">Open your programmes</Link></div>}</div>
    ...
    <SectionTitle title="Check-ins" href="/care/check-ins" label="Add entry"/>

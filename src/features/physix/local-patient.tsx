...
export function SavedPending({error,reload}:{error:string;reload:()=>void}) {
  const destination=usePathname();
  return <div className="px-empty" aria-busy={!error}><h2>{error?'Could not load your care.':'Loading your care…'}</h2>{error&&<><p role="alert">{error}</p><button className="button" onClick={reload}>Retry</button><Link className="px-text-link" href={careLoginHref(destination)}>Sign in again</Link></>}</div>;
}
...
function CheckInForm({data,onSaved}:{data:LocalAccount;onSaved:()=>void}){
 ...
 if(saved)return <div className="px-care-appointment" role="status"><h2>Check-in saved.</h2><p>Week of {dateLabel(week)} · difficulty {saved.difficulty}/10</p><p>{saved.body}</p><p className="px-note">Shared with the local test practitioner. This is not continuous or emergency monitoring.</p><Link className="px-text-link" href="/care/progress">View history<ArrowRight size={18}/></Link></div>;

...
The local booking UI uses Service → Time → Review with an explicit selected-time footer. Back preserves the draft; Close offers Keep booking or Leave booking. Confirmation is /care/appointments/:id after server acknowledgement, with ownership enforced before rendering. Upcoming/Past/Cancelled views retain records; past is not attendance. Calendar export contains generic local-test reminder data, not symptoms or a real video invitation.

Cancellation defaults focus to Keep appointment. While pending, repeated submission and dismissal are disabled; failure stays unconfirmed and retry retains its command ID. Successful cancellation persists and releases the time through the existing database command. Book again starts a fresh selection with only the public service/mode. Local UI hides cancellation after a visit starts; this is not a confirmed clinic cutoff policy. Real policies, notifications/refunds, live video and atomic rescheduling remain unimplemented. Never release the original time before a replacement is secured.

...
Implemented the hosted API, public offers/slots, verified-account reservation and cancellation, database overlap constraints, policy/price snapshots and retry identity; reused native booking and saved appointment screens. Source commit 50166c6. Email sign-in is deliberately gated by PHYSIX_AUTH_READY until the owner signs into the dashboard and redirect/template/SMTP/delivery checks are complete. No confirmed practitioner, schedule, durations, fees or policy were supplied: three browsing services exist, booking_enabled is false and actual slots are empty. End-to-end hosted sign-in/reservation and release readiness remain open. Full scope and setup instructions: HOSTED_SETUP.md.

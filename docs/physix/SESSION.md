...
Implemented the hosted API, public offers/slots, verified-account reservation and cancellation, database overlap constraints, policy/price snapshots and retry identity; reused native booking and saved appointment screens. Source commit 50166c6 deployed successfully to physix-sable.vercel.app. Live /api/physix/v1/offers returns the three dedicated database offers; slots returns an empty array. Browser service selection opens the real calendar and shows unpublished availability, with no fixture times. A follow-up removes the null-duration suffix and scopes policy acceptance to the selected offer/version. Email sign-in is deliberately gated by PHYSIX_AUTH_READY until the owner signs into the dashboard and redirect/template/SMTP/delivery checks are complete. No confirmed practitioner, schedule, durations, fees or policy were supplied: three browsing services exist, booking_enabled is false and actual slots are empty. End-to-end hosted sign-in/reservation and release readiness remain open. Full scope and setup instructions: HOSTED_SETUP.md.

Passed design, typecheck, scoped lint, 125 existing units plus the dedicated configuration isolation test, production build, isolated SQL booking/conflict/retry/ownership checks and diff check. Intentional security-definer entry points and private availability RLS are documented; unrelated public-coach and fitness grants are revoked. Informational inherited FK indexing suggestions remain. Automatic approval review blocked a local process with hosted config; it gave no more specific reason. Deployed checks are separate. Existing full browser-runner block remains.

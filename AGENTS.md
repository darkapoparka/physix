...
2. Bootstrap is complete. Continue this checkout; do not clone or create another alternative project. Work directly on `main`; do not create, use or push review/feature branches for PhysiX. Local verified commits and pushes to `darkapoparka/physix` `main` are authorized by the owner's 21 September 2026 direction. Keep the Gymaf donor unchanged. Provider provisioning, external payments and deployment still need explicit approval.
...
Home now uses a normal-flow white header, near-black emphasis, strong typographic introduction, one clinic/online booking pair and unboxed photographic service links. The owner rejected the pastel panels and image fades; do not restore them from historical screenshots. Existing full-size editorial imagery is decorative local preview content, not actual staff or clinical instruction. Keep 16px inputs, 14px action text, 44px standalone targets and 48px main actions. Follow DESIGN.md for composition; passing tests never substitutes for owner visual acceptance.

...
The owner rejected the white-polish card redesign. A white canvas is not permission to replace Fidelity's app composition, display typography or solid-colour panels with an outlined clinic catalogue. Keep white page space, the compact icon-only dock, logo-related green, and search high. Restore substantial media-first cards, solid mint/sage/sand surfaces and forest banners. Remove tiny thumbnail rows and the arbitrary featured/standard/compact bento variants.

Use CareCard for public service and patient-plan presentation; CareWeek adapts the original weekly-panel/calendar structure using stored schedule data only. Source references are M:/gym-fidelity/src/components/home.tsx and its today-card/today-copy/challenge-card/week-calendar CSS. The donor is read-only. The six existing cutouts are decorative discovery assets, not real staff or exercise instruction. Do not regenerate another full design set or rewrite application features for a surface change.
...
One Shell and visual family serve public discovery and private care. The owner rejected the centre-home styling at d5493ef, including its isolated green hero under the white header. The original Gymaf/Future references remain primary; Nike Training Club is a secondary media/composition reference, not a new framework or product. Read docs/physix/references/fitness-apps-20260920/README.md and inspect the actual saved screenshots before another visual change. Share tokens and interaction patterns, not one forced illustrated CareCard for every content role. No Services / By area selector. Preserve working booking, private care and the stable dock. The reference study is not owner approval of a new design; do not invent clinic media or identity.

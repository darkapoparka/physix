...
try{
 ...
 open('/login');click('.px-account-gate>.button.primary');wait('location.pathname==="/app"&&!!document.querySelector(".px-patient-grid")');
 ...
 click('nav[aria-label="Your care views"] a[href="/app/plans"]');
 wait('location.pathname==="/app/plans"&&document.querySelector("h1")?.textContent==="Your programmes"&&!!document.querySelector("input[type=search]")&&[...document.querySelectorAll("a[aria-label]")].some(e=>e.getAttribute("aria-label").startsWith("Open programme:"))');
 ...
 click('a[aria-label^="Open programme:"]');run('wait','dl');
 ...
 assert.equal(evaluate('document.querySelectorAll(".row-group a[href^=\\\"/app/workouts/\\\"]").length'),programme.scheduledIds.length);checks.push({name:'Programme detail contains its own sessions',passed:true});
 run('reload');run('wait','dl');assert.ok(evaluate('document.querySelector("h1").textContent==='+JSON.stringify(programme.title)));checks.push({name:'Programme deep link survives reload',passed:true});
 click('.row-group a[href^="/app/workouts/"]');run('wait','.px-plan-detail');check('Session overview links back to its programme','document.querySelector(".px-care-heading a").getAttribute("href")==='+JSON.stringify('/app/plans/'+programme.id));
 open('/app/schedule');run('wait','button[aria-label="Next week"]');
 ...
 open('/app/progress');run('wait','.px-care-stats');
 ...
 const routes=[['home','/'],['today','/app'],['library','/app/plans'],['programme','/app/plans/'+programme.id],['session-overview','/app/workouts/'+programme.scheduledIds[0]],['schedule','/app/schedule'],['progress','/app/progress']];

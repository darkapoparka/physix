...
try{
 ...
 check('Shared labelled capsule dock is 366 by 64 at 390px', '(()=>{const r=document.querySelector(".px-dock").getBoundingClientRect();return r.width===366&&r.height===64})()');shot('patient-390');
 ...
 check('The final exercise has one Finish action','[...document.querySelectorAll(".px-player button")].filter(b=>b.textContent.trim()==="Finish session").length===1');
 click('.px-player-actions>button:last-child');click('dialog .button.primary');check('Completion renders only after acknowledgement','document.querySelector("h1")?.textContent==="Session finished."');
 ...
 check('Booking is focused and no dock covers Continue','!document.querySelector(".px-dock")');const priorIds=me().appointments.map(a=>a.id);click('.px-booking-times>button:first-child');click('[data-booking-next]');run('wait','.px-review-card');shot('booking-review-390');click('[data-booking-next]');

...
try{
 ...
 const html=await (await fetch(base)).text();for(const forbidden of ['Synthetic patient demo','Assigned in care','/physix/movement.jpg','/physix/editorial/','GYMAF_REFERENCE_PREVIEW'])assert.equal(html.includes(forbidden),false,forbidden+' not rendered in production');

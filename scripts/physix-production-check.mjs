...
try{
 ...
 for(const route of ['/care','/care/programmes','/care/schedule','/care/progress','/care/profile','/care/appointments','/care/appointments/50000000-0000-4000-8000-000000000001','/care/check-ins','/care/sessions/10000000-0000-4000-8000-000000000001']){const r=await expect(route,307);assert.equal(r.headers.get('location'),'/login?returnTo='+encodeURIComponent(route));assert.match(r.headers.get('cache-control')||'',/private/);assert.match(r.headers.get('cache-control')||'',/no-store/);}
